...
new_node->prev = NULL; new_node->next = NULL; if (*head == NULL)
{
...
struct Node* temp = head; while (temp != NULL)
{
printf("%d", temp->data); if (temp->next != NULL)
{
printf(" <-> ");
}
temp = temp->next;
...
scanf("%d", &data); create(&head, data); break;

case 2:
printf("Enter the target node data before which to insert: "); scanf("%d", &target_data);
printf("Enter the data for the new node to insert: "); scanf("%d", &new_data);
insert_left(&head, target_data, new_data); break;

case 3:
printf("Enter the data of the node to delete: "); scanf("%d", &data);
delete_node(&head, data); break;

case 4:
printf("The current list is: "); display(head);
break;
